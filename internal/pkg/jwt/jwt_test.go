...
func TestGenerateAndParseTokenVersion(t *testing.T) {
	token, err := Generate("test-secret-at-least-32-characters", time.Hour, 42, "tester", 7)
	if err != nil {
...
func TestParseRejectsWrongSecretAndExpiredToken(t *testing.T) {
	token, err := Generate("test-secret-at-least-32-characters", -time.Minute, 1, "admin", 1)
	if err != nil {
		t.Fatalf("generate token: %v", err)
	}
	...
	fresh, err := Generate("test-secret-at-least-32-characters", time.Hour, 1, "admin", 1)
	if err != nil {
		t.Fatalf("generate token: %v", err)
	}

...
**演示中心**：内置完整业务闭环，支持一键自动、分步执行与聚光灯引导体验，并提供并发库存分配、模拟 PDA 并发拣货等工程验证实验，可查看真实单据、库存变化与操作证据。
...
- 工程验证内置三个实验：并发库存分配（可切换库存充足/不足，验证 FIFO 与防超卖）、模拟 PDA 并发拣货与重复扫码拦截、异步导入可靠性机制
- 实验包装请求只进入“实验记录”，不混入真实业务操作记录
- “运行状态与指标快照”展示 MySQL、Redis、连接池、协程、内存和业务数量，不用于压力测试或容量评估
- “业务证据”页面按业务对象、库存流水、业务操作记录三个维度，展示演示账号的真实单据、任务和流水
- 右下角状态条可快速查看业务证据和工程验证；“重置数据”和“退出并重置”只影响当前演示账号的数据

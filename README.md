...
**演示中心**：内置完整业务闭环、独立入库/出库/盘点演示、Excel 批量草稿、并发库存分配与模拟 PDA 并发拣货实验，可查看真实任务、库存变化和工程验证结果。
...
- 右下角“业务流程中心”可选择一键完整流程、批量创建草稿或并发出库分配实验
- 支持受控的并发出库分配实验，可配置并发张数和每张数量，记录成功/业务拒绝数量、库存锁定与 FIFO 分配结果
- “运行状态与指标快照”页面展示 MySQL、Redis、连接池、协程、内存和业务数量，不用于压力测试或容量评估

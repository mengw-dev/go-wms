...
- 模拟 Excel 批量入库：HR 可选择张数和每张数量，只创建草稿，后续自己批量提交、审核、收货、上架
- 模拟上游批量出库：HR 可选择张数和每张数量，只创建草稿，后续自己批量提交、审核、拣货
- 批量创建盘点单：只创建盘点草稿，HR 自己录入实盘数量并审核
- 并发出库测试：HR 可选择并发张数和每张数量，页面明确展示测试重点、总需求量、成功/拒绝数量和最终库存

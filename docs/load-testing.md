...
| 层级 | 目标 | 入口 |
| --- | --- | --- |
| 1. 正常业务流程 | 证明入库、上架、出库、盘点状态机完整可跑通 | 页面“业务流程中心 → 一键完整流程”，或 `k6 hr-flow.js` |
...
业务流程中心还提供“并发出库测试”，HR 可以选择并发张数和每张数量，页面会明确显示测试重点、总需求量、成功/拒绝数量和最终库存；“性能指标”页面会实时显示连接池、协程、内存和库存数量。

...
这个页面用于把 WMS 的“正常操作”和“并发压测”讲清楚，适合演示给 HR、面试官或技术评审。

## 三层验证

| 层级 | 目标 | 入口 |
| --- | --- | --- |
| 1. 正常业务流程 | 证明入库、上架、出库、盘点状态机完整可跑通 | 页面“演示控制台 → 一键完整流程演示”，或 `k6 hr-flow.js` |
| 2. 出库冒烟 | 验证一张出库单从 DRAFT 到 SHIPPED | `k6 outbound-e2e.js` |
| 3. 真实并发 | 验证库存锁、FIFO、防超卖、任务防超拣 | `k6 outbound-stress.js` / `pick-stress.js` |
...
推荐使用 A 实例做压测，B 实例保留给 HR 正常操作，避免压测数据影响演示体验。
...
这套流程和页面“一键完整流程演示”使用同一套核心业务服务，区别只是入口不同。

演示页面还提供“并发业务演示”，会在后端并发创建和处理出库单；演示控制台中的“性能指标”页面会实时显示连接池、协程、内存和库存数量。
...
- 压测不要直接打到真实生产数据库。
- 默认演示库存不足以支撑无限并发出库，业务失败通常是防超卖生效，不一定是系统故障。
